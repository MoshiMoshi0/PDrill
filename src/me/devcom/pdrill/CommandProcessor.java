...
public class CommandProcessor {
    ...
    public boolean process(CommandSender sender, String commandLabel,
        ...
        if (commandLabel.equalsIgnoreCase("pdscript")
                || commandLabel.equalsIgnoreCase("pdload")) {
                ...
                if (scriptString != "") {
                    ...
                    for (Integer i = 0; i < scriptArgsCount; i++) {
                        if (i + 2 < args.length) {
                            ...
                            sender.sendMessage(prefix + "This script needs ["
                                    + (scriptArgsCount - i)
                                    + "] more arguments");
                            return true;
                        }
                    ...
                    if (args.length - 2 > scriptArgsCount) {
                        sender.sendMessage(prefix
                                + "Too much arguments, ommiting ["
                                + ((args.length - 2) - scriptArgsCount)
                                + "] arguments");
                    }
            ...
            for (Integer id : drillIds) {
                ...
                if (drill == null) {
                    sender.sendMessage(prefix
                            + "Linking failed! No drill with id [" + id + "]");
                    fail = true;
                } else if (drill.linked) {
                    sender.sendMessage(prefix
                            + "Linking failed! Drill with id [" + id
                            + "] is allready linked");
                    fail = true;
                }
            }
            ...
            if (!fail) {
                ...
                if (fuel != null) {
                    ...
                    sender.sendMessage(prefix + "Linked " + drillIds.toString()
                            + " to [" + -id + "]");
                } else {
                    sender.sendMessage(prefix + "No such fuel [" + args[1]
                            + "] in database");
                }
            }
            return true;
            ...
            if (drillIds.size() > 0) {
                for (Integer id : drillIds) {
                    LinkDrill drill = plugin.drillManager
                            .getLinkDrillFromId(-id);
                    ...
                    drill.owner.sendMessage(prefix + "Unlinked "
                            + drillIds.toString() + " from [" + -id + "]");
            ...
            if (what.equalsIgnoreCase("drill")) {
                ArrayList<Drill> ownedDrills = drillManager
                        .getDrillsByOwner((Player) sender);

                sender.sendMessage("Drills: " + ownedDrills.size());
                sender.sendMessage("----------------------------");
                for (Drill entry : ownedDrills) {
                    Fuel fuel = entry.FuelMG.fuel();
                    sender.sendMessage("ID: " + entry.id + " ENABLED: "
                            + entry.enabled + " FUEL_ID: " + fuel.block_id
                            + " FUEL_LEFT: " + fuel.block_id);
                }
                sender.sendMessage("----------------------------");
            } else if (what.equalsIgnoreCase("fuel")) {
                ...
                for (Entry<Integer, Fuel> entry : configManager.fuels
                    ...
                    sender.sendMessage("     " + "drillAirSpeed: "
                            + fuel.drillAirSpeed);
                    sender.sendMessage("     " + "drillBlockSpeed: "
                            + fuel.drillBlockSpeed);
                    sender.sendMessage("     " + "fuelConsumptionBlockCount: "
                            + fuel.fuelConsumptionBlockCount);
                    sender.sendMessage("     " + "fuelConsumptionFuelCount: "
                            + fuel.fuelConsumptionFuelCount);
                }
                sender.sendMessage("----------------------------");
            ...
            if (what.equalsIgnoreCase("room")) {
                ArrayList<String> script = new ArrayList<String>();
                ArrayList<Integer> drillIds = getDrillIdsFromString(sender,
                        args[1]);
                ...
                if (drill != null) {
                    if (!drill.isVirtual) {
                        if (drill.blockPlaceManager == null) {
                            drill.blockPlaceManager = new BlockPlaceManager(
                                    drill, Integer.parseInt(args[1]), args[2]);
                            sender.sendMessage(prefix
                                    + "Place job created for drill [" + id
                                    + "]");
                        } else {
                            drill.blockPlaceManager.blockPlaceInterval = Integer
                                    .parseInt(args[1]);
                            drill.blockPlaceManager.placeDirection = args[2];
                            sender.sendMessage(prefix
                                    + "Place job updated for drill [" + id
                                    + "]");
                        }
                    } else {
                        sender.sendMessage(prefix + "Drill with id [" + id
                                + "] is LinkDrill!");
                    }
                } else {
    ...
    private ArrayList<Integer> getDrillIdsFromString(CommandSender sender,
            String string) {
        ...
        for (Integer i = 0; i < argCount; i++) {
            if (argCount >= 2 && i <= argCount - 3
                ...
                try {
                    ...
                    sender.sendMessage(prefix + idArgs[i] + " or "
                            + idArgs[i + 2] + " is not a valid number");
                    break;
                } finally {
                ...
                try {
                    ...
                    sender.sendMessage(prefix + idArgs[i]
    ...
    private void sendScripts(CommandSender sender, ArrayList<Integer> drillIds,
        ...
        for (Integer drillId : drillIds) {
            ...
            if (drill != null) {
                if (!drill.linked) {
                    if (drill.enabled) {
                        ...
                        sender.sendMessage(prefix + "[" + drillId
                                + "] Drill is disabled!");
                    }
                } else {
                    sender.sendMessage(prefix + "[" + drillId
            ...
            if (ret == 1) {
                sender.sendMessage(prefix + "[" + drillId
                        + "] Script compiling passed!");
            } else if (ret == 0) {
                sender.sendMessage(prefix + "[" + drillId
                        + "] Script compiling failed!");
            } else if (ret == 2) {
                sender.sendMessage(prefix + "[" + drillId
                        + "] Drill is allready running a script!");
            }
        }
    }
}
